...
class navList extends Vue {
  //横向标题高度
  @Prop({ default: 0, type: Number }) subtractHeight!: number;
  //视窗高度
  @Prop({ default: 0, type: Number }) viewClientHeight!: number;
  //近似等于超出边界时最大可拖动距离(px);
  @Prop({ default: 50, type: Number }) additionalY!: number;
  ...
  mounted() {
    let navList: HTMLElement = <HTMLElement>this.$refs.navList;
    //绑定事件
    navList.addEventListener("touchstart", this.navTouchStart);
    navList.addEventListener("touchmove", this.navTouchMove);
    navList.addEventListener("touchend", this.navTouchEnd);
  }
  ...
  beforeDestroy() {
    ...
    let navList: HTMLElement = <HTMLElement>this.$refs.navList;
    navList.removeEventListener("touchstart", this.navTouchStart);
    navList.removeEventListener("touchmove", this.navTouchMove);
    navList.removeEventListener("touchend", this.navTouchEnd);
  }

  @Watch("viewClientHeight")
  viewClientHeightChange() {
    this.viewClientHeight;
    let height = this.viewClientHeight - this.subtractHeight;
    (this.$refs.root as HTMLElement).style.height = `${height}px`;
  }
  ...
  get navListHeight() {
    if (this.domHeight > 0) {
      ...
      return (this.$refs.navList as HTMLElement).offsetHeight;
    }
  }
